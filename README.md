...
I selected **Option 2: Airport Simulation**.
The goal was to create a realistic environment where multiple aircraft manage their own cycles of taxiing, takeoff, and landing while allowing the user to observe the operations from various perspectives.
...
1. **Waypoint Navigation**: Each aircraft follows a pre-defined path of 68 waypoints using `Vector3.MoveTowards` for smooth translation.
2. **State Transitions**: The logic checks the `currentWaypointIndex` to trigger specific behaviors:
...
3. **Realistic Rotation**: All turns use `Quaternion.Slerp` to ensure the aircraft bank naturally toward their next target rather than snapping instantly.
4. **Acceleration/Deceleration**: Speed is never constant; I used `Mathf.MoveTowards` for all velocity changes to simulate the weight and inertia of a real plane.
